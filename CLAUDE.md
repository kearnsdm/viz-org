...
## The relay (LIVE)
URL: `https://www.devinkearns.com/viz/viz-relay.php` (the bare domain 301s to www — always use www; a 301 also downgrades POST to GET, so never rely on -L). Auth: the low-stakes relay key via the `X-Viz-Key` header; the key lives in Devin's password manager, each device's Backup/Sync dialog, and the server config. **v2: the relay IS the store** — documents live on its disk; nothing proxies to GitHub anymore. Actions: `GET ?action=ping` (revs overview), `GET/POST ?action=board|streams|reinforcement|analysis|inbox|chat`, `POST ?action=append` ({candidates:[…]}, deduped by id), `POST ?action=chat-post` (append one chat message; relay assigns id + timestamp), `POST ?action=migrate-from-gist` (one-time, already run; refuses without force). GETs return `X-Viz-Rev`; POSTs accept `X-Viz-Rev-Base` for compare-and-swap (omit to force — prefer sending it). Whole-file POSTs store the body verbatim after JSON validation (PHP re-encoding would corrupt `{}` to `[]`). Responses are `Cache-Control: no-store` — the host's default 2-day caching fed devices stale boards/revs until this was forced off. Relay burst limit 120 req/min → 429 + Retry-After. Migrated + verified live 2026-07-08 (CAS 409, {}-preservation, snapshots, conflict flow all tested end-to-end).

## The chat lane — board ↔ live Claude session (added 2026-07-11)
`viz-org-chat.json` carries short messages between the board and a live Claude session, so Devin can ask about a task without leaving the board. Message shape: `{id, at, from:"board"|"claude", text, taskId?, taskTitle?, replyTo?}` — id and timestamp are relay-assigned via `POST ?action=chat-post`; never write the file whole (that is for repair only). Append-only, capped at the newest 500 messages.
- **"Watch the board" procedure (Cowork/Code sessions):** poll `GET ?action=ping` every ~30s; when `revs.chat` moves, fetch `?action=chat`, answer any new `from:"board"` message with `chat-post` (`from:"claude"`, set `replyTo` and the same `taskId`), reading board/streams first for context. Announce when the watch starts and stops — no silent listeners.
- **Board UI status:** not built yet (deferred installment: Ask-Claude composer + per-task thread on the Task Sheet). Until it ships, post test messages with curl.
...
- **Frog toll (amended 2026-07-11 — no toll without a first step):** a 🔥 heavy task 3+ days without a sprint ⇒ any building session opens with a 10-minute sprint on the oldest frog. Before offering the sprint, Claude preps a 2–4 step micro-stream on that frog whose first step is a physical action ("open the doc, find the first comment"), and the offer names step 1, not the task. Waivable; waivers logged, never argued. Standing: any task flagged 🔥 gets a micro-stream offer within a day.
